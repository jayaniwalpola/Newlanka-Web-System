...
if (isset($_POST['submit'])) {
		...
		if (empty($a) and empty($b)) {
			...
			$query = "SELECT * FROM paction_db WHERE User_Name = '{$email}' AND Password = '{$hashed_password}' LIMIT 1";
					...
					$user = mysqli_fetch_assoc($result_set);
										...
										if ($user['Block_Unblock'] == 1) {
											$errors['User_Block_or_Unblock'] = 'Your Account Block';
													...
													$query = "UPDATE paction_db SET Last_Login = NOW() WHERE User_ID = '{$_SESSION['user_id']}' ";
													$result_set = mysqli_query($connection, $query);
														if (!$result_set) {
														...
														// redirect index.php
														header('Location: my_appointment.php?admin_login=successful login');
											}
													}  else {
...
<div class="card w-25 p-3 bg-light">
        ...
        <div class="card-body">
          ...
          <img src="../images/admin_login.png" class="rounded-circle w-50 bg-success" alt="Cinque Terre">
